  ...
  <div class="cart">
    <div class="cart-box" v-if="isLogin && cartList.length > 0">
      ...
      <div class="cart-title">
        <span class="all">共<i>{{ selCount || 0 }}</i>件商品</span>
      ...
      <div class="cart-list">
        <div class="cart-item" v-for="item in cartList" :key="item.goods_id">
          <van-checkbox icon-size="18" :value="item.isChecked" @click="changeChecked(item.goods_id)"></van-checkbox>
...
export default {
  ...
  methods: {
    ...
    changeAllCheck () {
      this.$store.commit('cart/changeAllCheck', !this.isAllChecked)
    },
    ...
    handleDel () {
      ...
      this.$store.dispatch('cart/delSel')
    },
    goPay () {
      if (this.selCount > 0) {
        this.$router.push({
          ...
          query: {
            mode: 'cart',
            cartIds: this.selCartList.map(item => { return item.id }).join(',')
          }
        })
      }
    }
  ...
  watch: {
    isEdit (val) {
      if (val) {
        this.$store.commit('cart/changeAllCheck', false)
      } else {
        this.$store.commit('cart/changeAllCheck', true)
      }
    }
  }
}
